...
</ style><script> document.addEventListener(
...
function checkPassword() {
  ...
  if (input == = password) {
    ...
    <script> let currentMode = 'auto';  // Начальный режим
let relay2State = false;  // Состояние реле вентилятора

function toggleRelay(relayNumber) {
  let relayStateElement =
      document.getElementById(\`relayState$\{ relayNumber }\`);
  let icon = relayNumber == = 1 ? document.getElementById("lightIcon")
                                : document.getElementById("fanIcon");
...
function toggleMode() {
  ...
  let settingsBlock =
      document.querySelector(".settings");  // Получаем блок настроек
  fetch('/setMode', {
    method : 'POST',
      ...
      .then(data =
                >
                {
                  ...
                  modeIcon.style.color =
                      currentMode == = 'auto' ? "#555" : "#e74c3c";
                  // Если включен авто-режим, сбрасываем состояния реле
                  if (currentMode == = 'auto') {
                    document.getElementById("relayState1").textContent =
                        "Выключено";
                    document.getElementById("relayState2").textContent =
                        "Выключено";
                    ...
                    document.getElementById("fanIcon").classList.remove(
                        "fan-rotate");
...
function updateInputState() {
  ...
  inputs.forEach(input = > { input.disabled = !isManualAndRelayOn; });
...
function saveLightingSettings() {
  ...
  const fanTemperatureThreshold =
      parseFloat(document.getElementById("fanTemperatureThreshold").value);
  const lightOnDurationMinutes = parseFloat(
      document.getElementById("lightOnDuration").value);  // в минутах
  const lightIntervalManualMinutes = parseFloat(
      document.getElementById("lightIntervalManual").value);  // в минутах

  // Проверяем, что все значения корректны
  if (isNaN(fanTemperatureThreshold) || isNaN(lightOnDurationMinutes) ||
      isNaN(lightIntervalManualMinutes)) {
    alert("Пожалуйста, заполните все поля корректными значениями.");
    return;
  }

  // Переводим время в миллисекунды
  const lightOnDuration =
      lightOnDurationMinutes * 60000;  // Преобразуем минуты в миллисекунды
  const lightIntervalManual =
      lightIntervalManualMinutes * 60000;  // Преобразуем минуты в миллисекунды

  const settings = {fanTemperatureThreshold, lightOnDuration,
                    lightIntervalManual};
...
document.addEventListener(
                     ...
                     .then(data = >
                                  {
                                    document.getElementById('temperature')
                                        .textContent = \`Температура : \$ {
                                      data.temperature
                                    }
                                    °C\`;
                                    document.getElementById('humidity')
                                        .textContent = \`Влажность
                                        : \${data.humidity} %\`;
                                    document.getElementById('soilMoisture')
                                        .textContent = \`Влажность почвы
                                        : \${data.soilMoisture} %\`;
                                  })
                     .catch(error =
                                > console.error('Error updating sensor data:',
                                                error));
               },
          1000);
...
</ script><script> function savePumpSettings() {
  const pumpStartHour =
      parseInt(document.getElementById("pumpStartHour").value);
  const pumpStartMinute =
      parseInt(document.getElementById("pumpStartMinute").value);
  ...
  if (isNaN(pumpStartHour) || isNaN(pumpStartMinute) || isNaN(pumpDuration) ||
      isNaN(pumpInterval)) {
  ...
  fetch("/updatePumpSettings", {
    ...
    headers : {"Content-Type" : "application/json"},
    body : JSON.stringify(
        {pumpStartHour, pumpStartMinute, pumpDuration, pumpInterval})
  })
      .then(response = > response.json())
...
fetch("/getPumpSettings")
    ...
    .then(data = >
                 {
                   document.getElementById("pumpStartHour").value =
                       data.pumpStartHour;
                   document.getElementById("pumpStartMinute").value =
                       data.pumpStartMinute;
                   document.getElementById("pumpDuration").value =
                       data.pumpDuration;
                   document.getElementById("pumpInterval").value =
                       data.pumpInterval;
                 })
...
function updateSensors(data) {
  ...
  // Обновление текста
  tempElement.textContent = \`Температура :\$ { data.temperature }
  °C\`;
...
app.post(
    '/updateSensorData', (req, res) = > {
      ...
      if (temperature != null && humidity != null && soilMoisture != null) {
        ...
        console.log(
      `Received sensor data:Temperature
...
app.get('/getMode', (req, res) = > { res.json({mode : currentMode}); });
...
app.post(
    '/toggleRelay/:relayNumber', (req, res) = > {
      ...
      if (!Number.isNaN(relayNumber) && sensorData[relayStateKey] != null) {
        if (currentMode == = 'manual') {
          ...
          console.log(
        `Relay ${relayNumber} toggled to $ {
            sensorData[relayStateKey] ? 'ON' : 'OFF'
          }` );
          res.json({[relayStateKey] : sensorData[relayStateKey]});
        } else {
          res.status(403).json(
              {error : 'Cannot toggle relay in automatic mode'});
        }
      } else {
...
app.get('/getPumpSettings', (req, res) = > { res.json(pumpSettings); });

// Эндпоинт для обновления настроек насоса
app.post(
    '/updatePumpSettings', (req, res) = > {
      const {pumpStartHour, pumpStartMinute, pumpDuration, pumpInterval} =
          req.body;

      if (pumpStartHour != null && pumpStartMinute != null &&
          pumpDuration != null && pumpInterval != null) {
...
app.get('/getLightingSettings', (req, res) = > { res.json(lightingSettings); });

// Эндпоинт для обновления настроек (в том числе для ручного управления)
// Эндпоинт для обновления настроек освещения
// Эндпоинт для обновления настроек освещения
app.post(
    '/updateLightingSettings', (req, res) = > {
      const {fanTemperatureThreshold, lightOnDuration, lightIntervalManual} =
          req.body;
      if (fanTemperatureThreshold != null && lightOnDuration != null &&
          lightIntervalManual != null) {
        ...
        console.log(`Lighting settings updated:fanTemperatureThreshold
                    : ${fanTemperatureThreshold}, lightOnDuration
                    : ${lightOnDuration}, lightIntervalManual
                    : $ { lightIntervalManual }`);
